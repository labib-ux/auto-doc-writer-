...
export const Dashboard: React.FC<DashboardProps> = ({ onLogout, onRepoSelect }) => {
  ...
  const loadData = async () => {
    ...
    // Once we have a user, fetch their isolated data
  ...
  const SidebarContent = () => (
    <>
        <div className="p-8">
          ...
          <span className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-black">Workspace: {user?.handle}</span>
        </div>
        ...
        <div className="p-6 border-t border-zinc-800">
           <div className="flex items-center gap-3 p-4 rounded-xl bg-black border border-zinc-800 mb-4">
               <div className="w-10 h-10 rounded-full bg-brand flex items-center justify-center font-black text-sm text-white shadow-lg shadow-brand/20 uppercase">
                 {user?.initials || '??'}
               </div>
               <div className="flex-1 min-w-0">
                 <p className="text-sm font-extrabold text-white truncate">{user?.name || 'Loading...'}</p>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative bg-black">
        ...
        <div className="flex-1 overflow-y-auto p-10 scroll-smooth">
            <div className="max-w-7xl mx-auto">
                {isLoading ? <div className="flex items-center justify-center h-64 text-brand font-black animate-pulse uppercase tracking-[0.2em]">Isolating Workspace...</div> : 
                                        ...
                                        <motion.div key={repo.id} layout onClick={() => onRepoSelect(repo.id)} className={`group relative rounded-2xl border-2 transition-all cursor-pointer ${repo.isActive ? 'bg-zinc-900 border-brand/40 shadow-xl' : 'bg-zinc-900/50 border-zinc-800 hover:border-zinc-700'} p-6`}>
                                            <div className="flex items-center justify-between gap-6">
                                                ...
                                                <div className="flex flex-col items-end gap-5">
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); setGeneratingRepo(repo); }} className="px-5 py-2.5 bg-brand text-white rounded-xl text-xs font-black uppercase shadow-lg active:scale-95 flex items-center gap-2 transition-all hover:bg-brand/80"><Play size={14} fill="currentColor" /> Generate</button>
                                                </div>
                                            </div>
                                        </motion.div>
                                    ))}
                                    ...
                                    <div className="space-y-10 relative flex-1">
                                        ...
                                        {activity.slice(0, 6).map((item) => (
                                            <div key={item.id} className="relative pl-12">
                                                <div className={`absolute left-0 top-0.5 w-6 h-6 rounded-full flex items-center justify-center border-4 border-zinc-900 z-10 ${item.type === 'error' ? 'bg-red-600' : item.type === 'push' ? 'bg-blue-600' : item.type === 'create' ? 'bg-green-600' : 'bg-brand'}`}>
                                                ...
                                                <div>
                                                    ...
                                                    <span className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">{item.time} {item.user && `• BY ${item.user.toUpperCase()}`}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
